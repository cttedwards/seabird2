...
#' @export
"extract.profiles" <- function(filename)
{
    ...
    header.split <- SeaBird.string.to.vector.of.words(file[parameter.lines[1] + 1])
    colnames <- c()
    k <- 1
    while (k < length(header.split)) {
        if (SeaBird.is.whole.number(header.split[k + 1])) {
    ...
    for (i in 1:n.profiles) {
        profile <- list()
        par.name <- SeaBird.string.to.vector.of.words(file[parameter.lines[i]])[7]
        ...
        for (j in 1:n.values)
            profile$values[j,  ] <- SeaBird.string.to.vector.of.numbers(subfile[j])
        ...
        for (j in 1:n.values)
            profile$parameters[j,  ] <- SeaBird.string.to.vector.of.numbers(subfile[j])
